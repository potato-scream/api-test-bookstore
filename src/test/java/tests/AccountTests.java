...
public class AccountTests extends TestBase {
  ...
  void successfulRegistrationTest() {
    ...
    step(
        ...
        () -> {
          ...
          usersToCleanup.add(
              new UserCleanupData(registrationResponse.getUserId(), tokenResponse.getToken()));
        });
    ...
    step(
        ...
        () -> {
          ...
          assertThat(errorResponse.getMessage())
              .contains("Passwords must have at least one non alphanumeric character");
        });
  }
  ...
  void unsuccessfulRegistrationUserAlreadyExistsTest() {
    ...
    RegistrationResponse registrationResponse =
        step("Setup: create a new user", () -> accountApi.registerUser(credentials));
    step(
        "Add user to cleanup list",
        () -> {
          GenerateTokenResponse tokenResponse = accountApi.generateToken(credentials);
  ...
  void successfulDeleteUserTest() {
    ...
    GenerateTokenResponse tokenResponse =
        step("Setup: generate token", () -> accountApi.generateToken(credentials));
  ...
  void deleteUserWithInvalidTokenTest() {
    ...
    ErrorResponse errorResponse =
        step(
            "Attempt to delete user with a random token",
            () ->
                accountApi.deleteUserExpectingError(
                    registrationResponse.getUserId(), data.getRandomString(170)));
  ...
  void successfulGenerationTokenTest() {
    ...
    GenerateTokenResponse generateTokenResponse =
        step("Perform token generation", () -> accountApi.generateToken(credentials));
    ...
    step(
        ...
        () ->
            accountApi.deleteUser(
                registrationResponse.getUserId(), generateTokenResponse.getToken()));
  }
